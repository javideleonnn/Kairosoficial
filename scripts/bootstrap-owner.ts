...
async function main() {
  ...
  if (userError || !userData.user) throw new Error(`No se pudo crear el usuario: ${userError?.message}`);
  ...
  const { data: ownerRole, error: roleError } = await supabase
    .from("roles").select("id").eq("key", "owner").single();
  if (roleError || !ownerRole) throw new Error(`No se encontró el rol owner: ${roleError?.message}`);

  const { error: memberError } = await supabase.from("organization_members").insert({
    organization_id: org.id, user_id: userData.user.id, role_id: ownerRole.id, status: "active",
  });
  if (memberError) throw new Error(`No se pudo crear la membresía: ${memberError.message}`);
